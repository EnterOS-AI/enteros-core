...
# enforcement that dedups them onto the molecule-ai-sdk SSOT without forcing
...
#
# NOTE ON SEEDING ORDER: this gate fetches the SSOT from molecule-ai-sdk main.
# Until the molecule-ai-sdk SSOT seed (provision-request/) is merged, the fetch
...
SSOT_URL="${SSOT_URL:-https://git.moleculesai.app/molecule-ai/molecule-ai-sdk/raw/branch/main/contracts/provision-request/provision-request.contract.json}"
...
# molecule-ai-sdk is public; send an explicit curl UA (the CF edge 403s the
...
if [ "$curl_status" -ne 0 ]; then
  echo "::error::Failed to fetch the molecule-ai-sdk provision-request SSOT from ${SSOT_URL} (curl exit $curl_status). Fail-closed. (If the molecule-ai-sdk seed has not merged yet, merge it first.)"
  exit 1
fi

# Both copies must be valid JSON (fail-closed on a corrupt SSOT or mirror).
if ! jq -e . "$TMP" >/dev/null 2>&1; then
  echo "::error::The molecule-ai-sdk provision-request SSOT did not parse as JSON (fetched from ${SSOT_URL}). Fail-closed."
  exit 1
fi
...
if cmp -s "${TMP}.local.canon" "${TMP}.ssot.canon"; then
  echo "OK -- core's mirror $LOCAL is the SAME CONTRACT as the molecule-ai-sdk provision-request SSOT (canonical-JSON identical)."
...
echo "::error::Core's mirror $LOCAL DRIFTED from the molecule-ai-sdk provision-request SSOT -- they are NOT the same contract (canonical-JSON differs)."
echo "Canonical diff (local mirror vs SSOT):"
diff -u "${TMP}.local.canon" "${TMP}.ssot.canon" || true
echo "Re-sync: align core's mirror to the molecule-ai-sdk SSOT at contracts/provision-request/provision-request.contract.json (the SSOT is canonical -- align the mirror to it, never the reverse)."
exit 1
