...
// SILENTLY rolls back. Repro: switch to `codex` while the model is a
...
//   - codex (registry-backed): includes gpt-5.5
const TEMPLATES = [
  {
    id: "t-claude-code",
    name: "Claude Code",
  ...
  },
  {
    ...
    registry_backed: true,
    registry_providers: [
      { name: "openai-subscription", display_name: "OpenAI Codex subscription" },
    ],
    registry_models: [
      { id: "gpt-5.5", name: "GPT 5.5", provider: "openai-subscription" },
      { id: "gpt-5.4", name: "GPT 5.4", provider: "openai-subscription" },
    ],
  },
];
...
describe("modelIdsForRuntime / defaultModelForRuntime helpers", () => {
  it("returns the registry model ids for a registry-backed runtime", () => {
    ...
    expect(defaultModelForRuntime(opt)).toBe("gpt-5.5");
  });
...
describe("ConfigTab — runtime change resets model (codex)", () => {
  it("(a) switching claude-code → codex resets the model to a valid codex model", async () => {
    ...
    // The model must reset to a codex-registered model, and the dropdown
    // must only offer codex models (no kimi).
    const modelSelectAfter = (await waitFor(() =>
      screen.getByTestId("model-select"),
    )) as HTMLSelectElement;
    await waitFor(() =>
      expect(modelSelectAfter.value).toBe("gpt-5.5"),
    );
    const optsAfter = Array.from(modelSelectAfter.options).map((o) => o.value);
    expect(optsAfter).toContain("gpt-5.5");
    ...
    expect(note.textContent).toMatch(/gpt-5\.5/);
    expect(note.textContent).toMatch(/moonshot\/kimi-k2\.6/);
  });

  it("(a') Save after the runtime switch PUTs a valid codex model — never the stale kimi model", async () => {
    wireClaudeCodeWorkspace();
    render(<ConfigTab workspaceId="ws-cc" />);
    ...
    // runtime PATCH carries codex; model PUT carries a codex model.
    await waitFor(() =>
  ...
  it("(b) an invalid (runtime, model) pair can't be submitted — Save is disabled", async () => {
    // Drive the form into a raw-YAML invalid pair: runtime=codex with a
    ...
    apiGet.mockImplementation((path: string) => {
      if (path === "/workspaces/ws-bad") return Promise.resolve({ runtime: "codex", tier: 2 });
      if (path === "/workspaces/ws-bad/model")
        return Promise.resolve({ model: "gpt-5.5", source: "workspace_secrets" });
    ...
    // Save & Restart must be disabled — the (codex, moonshot/kimi-k2.6)
