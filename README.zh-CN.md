...
- 通过标准 marketplace 流程安装：`claude plugin marketplace add https://git.moleculesai.app/molecule-ai/molecule-mcp-claude-channel.git` → `claude plugin install molecule@molecule-channel`（旧的 `Molecule-AI/…` GitHub 简写已失效——该组织于 2026-05-06 被封停，请使用完整的 Gitea URL）
