...
- drag-to-nest 团队构建
- empty state + 自托管首次运行设置界面（first-run scene）
- template palette
...
- 多租户友好：单次安装即可同时 watch 跨多个 Molecule 租户的 workspace —— 每个 workspace 在 `MOLECULE_WORKSPACES_JSON` 数组里各自带上自己的租户 URL（[规范配置形状](docs/guides/external-agent-registration.md#multiple-workspaces-from-one-local-mcp-bridge)）
...
# 启动 Postgres (:5432)、Redis (:6379)、MinIO (:9000/:9001)
# 以及 Langfuse (:3001)，全部挂在共享的
# `molecule-core-net` Docker 网络上。
#
...
1. 首次启动会自动种下组织的平台代理（concierge），画布随即进入全屏首次运行设置界面。
2. 在级联下拉框中依次选择 runtime、provider、model，填入对应的 API key —— **Enter OS Agent** 上线后会主动向你打招呼。
3. 之后可以直接与它对话，也可以在 empty state 中部署模板或创建 blank workspace。

无界面（headless）部署：在环境变量中设置 `MOLECULE_DEFAULT_RUNTIME`、`MOLECULE_LLM_DEFAULT_MODEL` 和对应的 provider key，首次启动即自动上线，设置界面不会出现。（`MOLECULE_SEED_PLATFORM_AGENT` 已移除 —— 自托管环境总是会自动种下 concierge。）
