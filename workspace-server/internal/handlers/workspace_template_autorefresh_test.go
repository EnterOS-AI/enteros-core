...
// (e.g. google-adk) whose workspace template is a cache MISS at provision
...
func TestResolveTemplateWithRefreshOnMiss(t *testing.T) {
	t.Run("hit: resolves without invoking refresh", func(t *testing.T) {
		...
		cacheDir := t.TempDir()
		writeTemplateDir(t, cacheDir, "google-adk", "google-adk")
		...
		got, err := h.resolveTemplateWithRefreshOnMiss(context.Background(), "google-adk", "google-adk")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		want := filepath.Join(cacheDir, "google-adk")
		if got != want {
	...
	t.Run("miss for known non-claude runtime: refresh is invoked and the template resolves", func(t *testing.T) {
		...
		// google-adk is initially absent from BOTH roots (cache miss). The
		...
		if seeded != "google-adk" {
			t.Fatalf("seeded runtime = %q, want google-adk (no claude-code substitution)", seeded)
		}
	})
	...
	t.Run("persistent miss for known non-claude runtime: FAIL LOUD, never claude-code", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "google-adk") {
			t.Fatalf("error %q does not name the runtime's own template", err.Error())
		}
		if strings.Contains(got, "claude-code") {
			t.Fatalf("resolved a claude-code path %q for a google-adk runtime — the exact #3211 regression", got)
		}
	})
	...
	t.Run("no refresh wired for known non-claude runtime: FAIL LOUD (unit-test/self-host degrade)", func(t *testing.T) {
		...
		// substitute it for google-adk even with no refresh mechanism.
		writeTemplateDir(t, configsDir, "claude-code-default", "claude-code")
		h := &WorkspaceHandler{
			configsDir: configsDir,
			cacheDir:   t.TempDir(),
	...
	t.Run("path traversal is rejected before any refresh", func(t *testing.T) {
		...
		if _, err := h.resolveTemplateWithRefreshOnMiss(context.Background(), "../escape", "google-adk"); err == nil {
