...
| `MEMORY_PLUGIN_URL` | `http://localhost:9100` (defaulted by the tenant entrypoint) | v2 memory plugin sidecar address. The sidecar is BUNDLED in the tenant image and its address is a constant, so nothing needs to configure this — `entrypoint-tenant.sh` defaults it and spawns the sidecar (task #4114). Previously every provisioner had to re-declare the constant; the cloud user-data paths did and the local-docker path did not, so docker-provisioned tenants booted with no memory and every memory call 503'd. Set `MEMORY_PLUGIN_DISABLE=1` to skip the sidecar. A LOOPBACK value (including the `http://localhost:9100` the CP injects) means the bundled sidecar; any other value means an EXTERNAL plugin you run yourself, and the entrypoint then starts no sidecar and leaves your URL untouched. If the bundled sidecar never becomes healthy the tenant DEGRADES (boots; memory endpoints 503) — set `MEMORY_PLUGIN_REQUIRED=1` to make that fatal instead. Requiring memory is a separate flag on purpose: the CP sets `MEMORY_PLUGIN_URL` on every cloud tenant, so the URL cannot carry that intent. Applies to the TENANT image (`Dockerfile.tenant` / `entrypoint-tenant.sh`); the standalone `workspace-server/Dockerfile` still spawns its sidecar only when the URL is set. |
