...
**Update (open_url navigation model — DECIDED by test, 2026-07-27):** empirically confirmed that Chromium in `--kiosk` mode **ignores `Ctrl+L`** (the omnibox is disabled), so `open_url` cannot reuse the screenshot/input primitives (title stayed `about:blank`, screenshot unchanged after ctrl+L→type→Enter). Decision: `open_url` navigates via the **Chrome DevTools Protocol** — launch Chromium with `--remote-debugging-port=<localhost-only>` and add a control-server `POST /navigate {url}` (→ gateway route → runtime tool) that issues `Page.navigate`. This keeps the pinned single kiosk window (no new tab/window) and works with the URL bar hidden. Scoped as the immediate additive follow-up PR after the core stack lands; the agent is already fully functional (screenshot + click + type + key + scroll) without it.

**Update (PR-review blockers found + fixed, 2026-07-27):** a dispatched reviewer traced the full platform→sidecar path and caught 3 real defects my component-level tests missed (each fails closed, none a security hole, but the feature was non-functional once enabled):
- **B1** — `StartDesktop` created the container with **no `Env`**, so the control server hit `log.Fatal("DESKTOP_CONTROL_TOKEN required")` and the sidecar died at boot (the e2e passed only because it injected the token by hand). Fixed: the provisioner now derives `DESKTOP_CONTROL_TOKEN` from the shared secret (identical to the gateway's TokenResolver) and sets `DESKTOP_WIDTH/HEIGHT`; `DeriveDesktopControlToken` moved to the provisioner package. Env asserted in the provisioner test.
- **B2** — the gateway passively checked `AgentHoldsControl`, but **nothing ever granted the agent a lease**, so every `/input` returned 409. Fixed: `AcquireAgentControl` actively takes/refreshes a `controller='agent'` lease UNLESS a human holds an unexpired lock (capability-IS-authorization; yields to a preempting human). Gateway `Input` now acquires-or-yields. **Verified vs PG16** (acquire → refresh → yield-to-human → reclaim-after-lapse).
- **B3** — the sidecar is on `wsnet-<id>` only, so the platform (on the shared net) couldn't resolve `wsdesk-<id>` by name. Fixed: `StartDesktop` attaches the **platform's own container** to each per-workspace network (self-id from `HOSTNAME`/`MOLECULE_DESKTOP_PLATFORM_CONTAINER`); `StopDesktop` detaches before removing the net. Sidecar stays off molecule-core-net (isolation intact); only the trusted platform joins. Attach asserted in the provisioner test.

**Update (2026-07-28 — STRUCTURAL egress isolation; feature now DEFAULT-ON, no flags):** the two enablement flags were friction guarding a real hole (B1). Rather than keep an operator firewall + interlock, the isolation is now enforced by topology so default-on is safe by construction:
- Each sidecar runs on a per-workspace **INTERNAL** Docker network (`Internal: true`) — **no egress of its own**. Its only route out is a per-workspace **egress proxy** (`cmd/desktop-egress-proxy`, run from the same image) that **denies all private/link-local/loopback destinations** (RFC-1918, `169.254/16` = metadata + host gw, loopback, IPv6 ULA) and allows only the public internet. Chromium is pointed at it via `--proxy-server=$DESKTOP_PROXY`.
- **Verified end-to-end on real Docker 29.5.3 (2026-07-28):** sidecar has NO default route; direct internet/metadata blocked; the proxy returns 403 for metadata (GET+CONNECT) and private 10.x, 200 for example.com; the **real Chromium navigated example.com through the proxy** (screenshot 15064→27699, title "Example Domain"). Proxy deny logged with reasons. Unit tests cover the deny logic + handler.
- **Both flags removed:** `MOLECULE_DESKTOP_ENABLED` + `MOLECULE_DESKTOP_EGRESS_CONFIRMED` are gone; the feature wires automatically wherever a local Docker provisioner exists (`MOLECULE_DESKTOP_DISABLE=true` to opt out). Retired `scripts/desktop-egress-firewall.sh`; rewrote the operator doc (no required step). **This makes the feature ON in production once the image is published + platform redeployed — no separate operator egress action.**
- **Security review (2026-07-28): APPROVE — no bypass found.** A reviewer empirically attacked the proxy deny logic (v4-mapped IPv6, NAT64, 6to4, DNS-rebinding, redirects, decimal/octal, `http_proxy` env leak, CONNECT-to-private, `0.0.0.0`/loopback/multicast/ULA) and the topology — all fail-closed; the internal net makes the proxy the only route out. Proxy given a 256MB memory cap per the review. **Residual (threat-model note, not infra exposure):** the platform joins each per-workspace net (B3) so the gateway can reach the sidecar by name, so a fully-compromised sidecar can reach the workspace-server API directly (auth-gated by workspace/admin bearer — not a path to infra/metadata). Optional `MOLECULE_DESKTOP_EGRESS_NETWORK` pins a dedicated internet-only bridge on shared hosts.
