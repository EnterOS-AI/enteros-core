...
AGENT_ID=$(curl -s "$BASE/workspaces" ${ADMIN_AUTH[@]+"${ADMIN_AUTH[@]}"} | python3 -c "
import sys, json
...
AGENT_AUTH=(-H "Authorization: Bearer $AGENT_TOKEN")
...
for _ in 1 2 3 4 5 6; do
  R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity?type=a2a_receive" "${AGENT_AUTH[@]}")
...
R=$(curl -s -X POST "$BASE/workspaces/$AGENT_ID/activity" "${AGENT_AUTH[@]}" -H "Content-Type: application/json" \
...
R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity?type=error" "${AGENT_AUTH[@]}")
...
R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity?type=task_update" "${AGENT_AUTH[@]}")
check "Filter task_update activities" 'document analysis' "$R"

# Test 11: Filter by agent_log
R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity?type=agent_log" "${AGENT_AUTH[@]}")
check "Filter agent_log activities" 'inference' "$R"

# Test 12: Total count includes all types
R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity" "${AGENT_AUTH[@]}")
...
R=$(curl -s "$BASE/workspaces/$AGENT_ID/activity?limit=2" "${AGENT_AUTH[@]}")
...
R=$(curl -s "$BASE/workspaces" ${ADMIN_AUTH[@]+"${ADMIN_AUTH[@]}"})
...
R=$(curl -s -X POST "$BASE/workspaces" ${ADMIN_AUTH[@]+"${ADMIN_AUTH[@]}"} -H "Content-Type: application/json" -d '{"name":"Activity Test Workspace","tier":1,"runtime":"external","external":true}')
TEMP_ID=$(echo "$R" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])")

# Test 20: New workspace has empty activity
R=$(curl -s "$BASE/workspaces/$TEMP_ID/activity" ${HUMAN_AUTH[@]+"${HUMAN_AUTH[@]}"})
check "New workspace has no activity" '[]' "$R"

# Test 21: Report activity to temp workspace
curl -s -X POST "$BASE/workspaces/$TEMP_ID/activity" ${HUMAN_AUTH[@]+"${HUMAN_AUTH[@]}"} -H "Content-Type: application/json" \
...
R=$(curl -s "$BASE/workspaces/00000000-0000-0000-0000-000000000000/activity" ${HUMAN_AUTH[@]+"${HUMAN_AUTH[@]}"})
